...
@implementation CreateNewEventGameday
@synthesize selection, teamId, eventDateTime, haveTeamList, teamList, teamListFailed, error, createButton, createMultipleButton,
createSingleLabel;

-(void)viewWillAppear:(BOOL)animated{
    
    [TraceSession addEventToSession:@"CreateNewEventGameday - View Will Appear"];
	...
	if (self.teamListFailed) {
		...
		if ([self.teamList count] == 0) {
			...
			if (coordTeam) {
				...
				NSDate *finalDate = self.eventDateTime.date;
	...
	if (self.teamListFailed) {
		...
		if ([self.teamList count] == 0) {
			...
			if (coordTeam) {
				...
				NSDate *selectedDate = self.eventDateTime.date;
				
				SelectTeamCal *tmp = [[SelectTeamCal alloc] initWithStyle:UITableViewStyleGrouped];
				
				tmp.teams = coordTeams;
				tmp.finalDate = selectedDate;
