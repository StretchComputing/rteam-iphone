...
- (NSDate *) date
{
	...
	NSDate * newDate = (__bridge_transfer NSDate *) CFDateCreate( NULL, [self absoluteTime] );
	return newDate;
}
...
- (void) setDate:(NSDate *)newDate
{
	...
	[self setAbsoluteTime: CFDateGetAbsoluteTime( (__bridge CFDateRef)newDate )];
}
...
- (NSString *) dayName
{
	...
	return _dayNames[(currentDay-1)];
}

- (NSString *) monthName
{
	unsigned currentMonth = ([self month] - 1);
	return _monthNames[currentMonth];
}
...
- (void) adjustDays: (int)days
{
	...
	const char format[] = "d"; //unsigned char
...
- (void) adjustMonths: (int)months
{
	...
	{
		const char dFormat[] = "d"; //unsigned char
		CFCalendarAddComponents ( [self calendar], &newTime, 0, dFormat, (months * 30) );
        
	} else {
		
		const char mFormat[] = "M"; //unsigned char
...
- (void) adjustYears: (int)years
{
	...
	const char format[] = "y"; //unsigned char
...
- (void) moveToFirstDayOfMonth
{
	...
	const  char format[] = "yMdHms"; //unsigned char
