...
messageActivity, messageActivityLabel, doneInbox, doneOutbox, gameId, callReload, messageField, allMessages, errorLabel, errorString, noMessagesLabel,
tmpAllMessages, startDate, hideTab, infoText;
...
-(void)viewDidLoad{
	...
	self.messageField.placeholder = [NSString stringWithFormat:@"Enter a message for game on %@...", stringDate];
...
-(void)getAllMessages{
	...
	if (![token isEqualToString:@""]){
		...
		NSDictionary *response = [ServerAPI getMessageThreads:token :self.teamId :@"all" :self.gameId :@"game" :@"message" :@"all"];
...
didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	ViewGameChatterMessage *next = [[ViewGameChatterMessage alloc] init];
...
-(void)createMessage{
	...
	if (![token isEqualToString:@""]){	
		...
		NSDictionary *response = [ServerAPI createMessageThread:token :self.teamId :@"Game Message" :self.messageField.text :@"plain" :self.gameId :@"game" :@"false" :[NSArray array] :[NSArray array] :@"" :@"false" :@""];
