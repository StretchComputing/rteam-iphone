...
selectRecipButton, replyAlert, keyboardToolbar;
	...
	if (self.toTeam || [self.recipients count] > 0 || self.isReply) {
	  ...
	  if (![self.messageField.text isEqualToString:@""] &&
	    ...
	    [self performSelectorInBackground:@selector(runRequest) withObject:nil];
		  ...
		  self.errorMessage.text = @"*You must enter a Message";
	  }
...
- (void)runRequest {
	...
	if (![token isEqualToString:@""]){	
		...
		if ([self.subjectField.text isEqualToString:@""]) {
			theSubject = @"(no subject)";
		}else {
			theSubject = self.subjectField.text;
		}

      
	response = [ServerAPI createMessageThread:token :self.sendTeamId :theSubject :self.messageField.text :messageType :self.eventId 
...
- (void)didFinish{
	...
	if (self.createSuccess){
		...
		}else if ([self.origLoc isEqualToString:@"GameTabs"]) {
		...
		}else if ([self.origLoc isEqualToString:@"PracticeTabs"]) {
		...
		}else if ([self.origLoc isEqualToString:@"EventTabs"]) {
		...
		}else if ([self.origLoc isEqualToString:@"PlayerProfile"]){
		
			Player *cont = [tempCont objectAtIndex:tempNum];
			cont.messageSent = true;
			[self.navigationController popToViewController:cont animated:YES];
			
			
		}else if ([self.origLoc isEqualToString:@"ConfirmMessageDetail"]){
			
			ConfirmMessageDetail *cont = [tempCont objectAtIndex:tempNum];
			cont.messageSent = true;
			[self.navigationController popToViewController:cont animated:YES];
			
			
		}else if ([self.origLoc isEqualToString:@"ConfirmPollDetail"]){
			
			ConfirmPollDetail *cont = [tempCont objectAtIndex:tempNum];
			cont.messageSent = true;
			[self.navigationController popToViewController:cont animated:YES];
			
			
		}else if ([self.origLoc isEqualToString:@"reply"]) {
		...
		}else{ //MessagesTabs (dont go anywhere)

			self.errorMessage.textColor = [UIColor greenColor];
			self.errorMessage.text = @"Message Sent Successfully!";
			self.subjectField.text = @"";
			self.messageField.text = @"Enter your message here...";
			self.recipientLabel.text = @"";
