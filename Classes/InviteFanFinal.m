	...
	@autoreleasepool {
        ...
        for (int i = 0; i < [self.emailArray count]; i++) {
            ...
            if (![tmpMember.role isEqualToString:@""]) {
                tmpDictionary[@"participantRole"] = @"fan";
            }
	...
	if ([self.errorString isEqualToString:@""]) {
		...
		if (theCount >= 0) {
			if ([CurrentTeamTabs class] == [tmpViews[[tmpViews count] - 2] class]) {
				...
				tmp.phoneOnlyArray = self.phoneOnlyArray;
				[self.navigationController popToViewController:tmpCont animated:NO];
			}else if ([MyTeams class] == [tmpViews[[tmpViews count] - 2] class]) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView
	...
	if (isEmpty) {
        ...
        if (memberRow) {
            ...
            NewMemberObject *tmp = (self.emailArray)[row];
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
   ...
   if (alertView == self.miniMultipleEmailAlert){
        ...
        if (buttonIndex != 0) {
            ...
            self.emailText.text = (self.multipleEmailArray)[buttonIndex-1];
        ...
        if (buttonIndex != 0) {
            ...
            self.phoneText.text = (self.multiplePhoneArray)[buttonIndex-1];
