...
@implementation FastUpdateStatus
...
eventsArray, eventsError, eventsNowSuccess, currentEventIndex, teamLabel, noEventsText;
...
- (UITableViewCell *)tableView:(UITableView *)tableView
	...
	rightArrow.frame = CGRectMake(255, 0, 45, 45);
	...
	leftArrow.frame = CGRectMake(0, 0, 45, 45);
	...
	if (row == 0) {
		optionLabel.text = @"• I plan on attending.";
	}else if (row == 1) {
		optionLabel.text = @"• I'm on my way.";
	}else if (row == 2) {
		optionLabel.text = @"• Sorry, can't make it.";
	}else if (row == 3) {
		optionLabel.text = @"• I'm lost.";
	}else if (row == 4) {
		optionLabel.text = [NSString stringWithFormat:@"I'll be there in %@ minutes.", self.currentInterval];
...
-(void)rightCell{
	...
	if ([self.currentInterval isEqualToString:@"5"]) {
		self.currentInterval = @"10";
	}else if ([self.currentInterval isEqualToString:@"10"]) {
		self.currentInterval = @"15";
	}else if ([self.currentInterval isEqualToString:@"15"]) {
		self.currentInterval = @"30";
	}else if ([self.currentInterval isEqualToString:@"30"]) {
		self.currentInterval = @"5";
	}
...
didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if (self.eventsNowSuccess && ([self.eventsArray count] > 0)) {
		...
		if (row == 0) {
			...
			message = [NSString stringWithFormat:@"I'll be there in %@ minutes.", self.currentInterval];
		}
		
		FastUpdateStatus2 *tmp = [[FastUpdateStatus2 alloc] init];
