...
//  rTeam
//
//  Created by Nick Wroblewski on 2/2/10.
...
inPseudoEditMode, pseudoSelectedArrayZero, pseudoSelectedArrayOne, editDone, deleteSuccess, threadIdsToArchiveInbox, deleteButton,
...
messageActivity, messageActivityLabel, messageDeleteView, doneInbox, doneOutbox, gameId, callReload;
...
-(void)getAllMessages{
	...
	if (![token isEqualToString:@""]){
		...
		NSDictionary *response = [ServerAPI getMessageThreads:token :self.teamId :@"all" :self.gameId :@"game" :@"both" :@"active"];
	...
	//NSLog(@"Count: %@", [self.results count]);
...
-(void)sendMessage{
	...
	nextController.eventId = self.gameId;
	nextController.eventType = @"game";
	nextController.origLoc = @"GameTabs";
...
- (UITableViewCell *)tableView:(UITableView *)tableView
	...
	if (!wasViewed) {
		...
		subjLabel.frame = CGRectMake(45, 5, 163, 22);
